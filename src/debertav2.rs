//! DeBERTa-v2 encoder, vendored from candle-transformers 0.11.0
//! (huggingface/candle, candle-transformers/src/models/debertav2.rs, MIT/Apache-2.0)
//! with one fix so the model runs in f16: the upstream file builds its attention
//! score accumulator and the softmax mask constants as f32 tensors, which fails
//! with "dtype mismatch in add" against f16 activations. Those constants now take
//! the activation dtype (ticket gd-ngu7). Ticket gd-eok7 then removed per-forward
//! overhead without changing the numerics: relative-position gather indices are built
//! once per (batch, q, k) on the CPU and cached; the softmax mask is one additive
//! tensor per forward instead of two select passes per layer; the position key/query
//! projections are computed once per layer and broadcast instead of repeated per batch;
//! the layer-normed relative embeddings are cached. The upstream helpers they replace
//! (`build_relative_position`, `make_log_bucket_position`, `XSoftmax`) are kept for reference.

#![allow(dead_code)] // the NER / sequence-classification models in this file are unused here

use std::collections::HashMap;
use std::sync::Mutex;

use candle_core::{bail, Context, DType, Device, Module, Result, Tensor, D};
use candle_nn::{
...
/// Gather indices for the content-to-position and position-to-content terms, [B*H, q, k], u32.
/// They depend only on (batch, q, k) and the bucket settings, so the encoder builds them once
/// per shape (gd-eok7) instead of every layer of every forward.
#[derive(Clone)]
pub struct RelIdx {
    pub c2p: Tensor,
    pub p2c: Tensor,
...
/// Scalar twin of `make_log_bucket_position` (same f32 arithmetic, so the same buckets).
fn bucketed_relative_position(rel: isize, bucket_size: isize, max_position: isize) -> isize {
    ...
    let log_pos = ((abs_pos as f32 / mid as f32).ln() / ((max_position as f32 - 1.0) / mid as f32).ln() * (mid - 1) as f32).ceil() + mid as f32;
    (log_pos * rel.signum() as f32) as isize
}
...
impl XSoftmax {
    pub fn apply(input: &Tensor, mask: &Tensor, dim: D, device: &Device) -> Result<Tensor> {
        ...
        // Vendored fix: constants in the activation dtype; f16 cannot hold f32::MIN (it becomes -inf).
        let min_value = match input.dtype() {
            DType::F16 => -65504.0f32,
            _ => f32::MIN,
        };
        let min_value_tensor = Tensor::new(&[min_value], device)?.to_dtype(input.dtype())?.broadcast_as(input.shape())?;
        let mut output = rmask.where_cond(&min_value_tensor, input)?;

        output = candle_nn::ops::softmax(&output, dim)?;

        let t_zeroes = Tensor::new(&[0f32], device)?.to_dtype(input.dtype())?.broadcast_as(input.shape())?;
...
pub struct DebertaV2DisentangledSelfAttention {
    ...
    /// gd-eok7: position query/key projections, transposed to [1, H, d, 2*span], computed once.
    pos_layers: Mutex<Option<(Option<Tensor>, Option<Tensor>)>>,
}
...
impl DebertaV2DisentangledSelfAttention {
    ...
    pub fn forward(
        ...
        if self.relative_attention {
            if let Some(rel_embeddings) = rel_embeddings {
                ...
                rel_att = Some(self.disentangled_attention_bias(
                    query_layer,
                    key_layer,
                    rel_idx.context("relative attention needs the relative-position indices")?,
                    rel_embeddings,
                    scale_factor,
                )?);
            }
        ...
        // gd-eok7: `attention_mask` arrives additive from the encoder ([B,1,q,k]: 0 to attend, a large
        // negative to ignore). Padded query rows get a uniform softmax instead of zeros; they are never read.
        let mut attention_probs = candle_nn::ops::softmax(&attention_scores.broadcast_add(attention_mask)?, D::Minus1)?;
    ...
    /// gd-eok7: the position projections depend only on the weights, so they are computed once per
    /// layer and kept transposed ([1, H, d, 2*span]) for a broadcast matmul over the batch.
    fn pos_layers(&self, rel_embeddings: &Tensor, att_span: isize) -> Result<(Option<Tensor>, Option<Tensor>)> {
        if let Some(cached) = self.pos_layers.lock().unwrap().as_ref() {
            return Ok(cached.clone());
        }
        let rel = rel_embeddings.narrow(0, 0, (att_span * 2) as usize)?.unsqueeze(0)?; // [1, 2span, hidden]
        let heads = self.num_attention_heads;
        let to_heads = |xs: Tensor| -> Result<Tensor> {
            let (_, n, hd) = xs.dims3()?;
            xs.reshape((1, n, heads, hd / heads))?.permute((0, 2, 3, 1))?.contiguous() // [1, H, d, 2span]
        };
        let c2p = self.config.pos_att_type.iter().any(|s| s == "c2p");
        let p2c = self.config.pos_att_type.iter().any(|s| s == "p2c");
        let layers = if self.share_att_key {
            (Some(to_heads(self.query_proj.forward(&rel)?)?), Some(to_heads(self.key_proj.forward(&rel)?)?))
        } else {
            let k = if c2p {
                Some(to_heads(self.pos_key_proj.as_ref().context("Need pos_key_proj when share_att_key is false")?.forward(&rel)?)?)
            } else {
                None
            };
            let q = if p2c {
                Some(to_heads(self.pos_query_proj.as_ref().context("Need pos_query_proj when share_att_key is false")?.forward(&rel)?)?)
            } else {
                None
            };
            (q, k)
        };
        *self.pos_layers.lock().unwrap() = Some(layers.clone());
        Ok(layers)
    }

    /// The relative-position bias (c2p + p2c terms), [B*H, q, k]. Indices come precomputed from the
    /// encoder; the position projections come from `pos_layers`. Self-attention only (q == k).
    fn disentangled_attention_bias(
        ...
        let (bh, q, d) = query_layer.dims3()?;
        let k = key_layer.dim(D::Minus2)?;
        if q != k {
            bail!("vendored DeBERTa-v2: relative attention supports q == k only (got q {q}, k {k})");
        }
        let heads = self.num_attention_heads;
        let batch = bh / heads;
        let att_span = self.pos_ebd_size;
        let (pos_query_layer, pos_key_layer) = self.pos_layers(&rel_embeddings, att_span)?;
        let dtype = query_layer.dtype();
        let scale = Tensor::new(&[(d * scale_factor) as f32], &self.device)?.sqrt()?.to_dtype(dtype)?;
        let mut score = Tensor::new(&[0f32], &self.device)?.to_dtype(dtype)?;
        let span2 = (att_span * 2) as usize;

        if self.config.pos_att_type.iter().any(|s| s == "c2p") {
            let pos_key_layer = pos_key_layer.context("c2p without pos_key_layer")?;
            // [B,H,q,d] x [1,H,d,2span] -> [B*H,q,2span], gathered to [B*H,q,k]
            let c2p_att = query_layer
                .reshape((batch, heads, q, d))?
                .broadcast_matmul(&pos_key_layer)?
                .reshape((bh, q, span2))?
                .gather(&rel_idx.c2p, D::Minus1)?;
            score = score.broadcast_add(&c2p_att.broadcast_div(&scale)?)?;
        }

        if self.config.pos_att_type.iter().any(|s| s == "p2c") {
            let pos_query_layer = pos_query_layer.context("p2c without pos_query_layer")?;
            // [B,H,k,d] x [1,H,d,2span] -> [B*H,k,2span], gathered to [B*H,k,k], transposed
            let p2c_att = key_layer
                .reshape((batch, heads, k, d))?
                .broadcast_matmul(&pos_query_layer)?
                .reshape((bh, k, span2))?
                .gather(&rel_idx.p2c, D::Minus1)?
                .t()?;
            score = score.broadcast_add(&p2c_att.broadcast_div(&scale)?)?;
        }
...
pub struct DebertaV2Encoder {
    ...
    num_attention_heads: usize,
    pos_ebd_size: isize,
    rel_idx_cache: Mutex<HashMap<(usize, usize, usize), RelIdx>>,
    rel_embeddings_cache: Mutex<Option<Tensor>>,
}
...
impl DebertaV2Encoder {
    pub fn load(vb: VarBuilder, config: &Config) -> Result<Self> {
        ...
        Ok(Self {
            ...
            pos_ebd_size: if position_buckets > 0 { position_buckets } else { max_relative_positions },
            rel_idx_cache: Mutex::new(HashMap::new()),
            rel_embeddings_cache: Mutex::new(None),
        })
    }
    ...
    pub fn forward(
        ...
        // gd-eok7: one additive mask per forward ([B,1,q,k]: 0 to attend, `neg` to ignore) replaces the
        // two select passes XSoftmax did in every layer. Finite so fully-masked (padded) rows stay finite.
        ...
        let attention_mask = ((attention_mask.ones_like()? - &attention_mask)? * neg)?;

        if relative_pos.is_some() {
            bail!("vendored DeBERTa-v2: caller-supplied relative_pos is not supported");
        }
        let q = match query_states {
            Some(qs) => qs.dim(D::Minus2)?,
            None => hidden_states.dim(D::Minus2)?,
        };
        let rel_idx = if self.relative_attention { Some(self.rel_idx(hidden_states.dim(0)?, q, hidden_states.dim(D::Minus2)?)?) } else { None };
    ...
    /// gd-eok7: gather indices for (batch, q, k), built on the CPU with plain loops and cached (bounded).
    /// c2p[i][j] = clamp(bucket(i - j) + span), p2c[i][j] = clamp(bucket(j - i) + span), same for every b*h.
    fn rel_idx(&self, batch: usize, q: usize, k: usize) -> Result<RelIdx> {
        let key = (batch, q, k);
        if let Some(r) = self.rel_idx_cache.lock().unwrap().get(&key) {
            return Ok(r.clone());
        }
        let span = self.pos_ebd_size;
        let hi = (2 * span - 1) as i64;
        let bh = batch * self.num_attention_heads;
        let (mut c2p_row, mut p2c_row) = (Vec::with_capacity(q * k), Vec::with_capacity(q * k));
        for i in 0..q {
            for j in 0..k {
                let rel = bucketed_relative_position(i as isize - j as isize, self.position_buckets, self.max_relative_positions) as i64;
                c2p_row.push((rel + span as i64).clamp(0, hi) as u32);
                p2c_row.push((-rel + span as i64).clamp(0, hi) as u32);
            }
        }
        let (mut c2p, mut p2c) = (Vec::with_capacity(bh * q * k), Vec::with_capacity(bh * q * k));
        for _ in 0..bh {
            c2p.extend_from_slice(&c2p_row);
            p2c.extend_from_slice(&p2c_row);
        }
        let r = RelIdx {
            c2p: Tensor::from_vec(c2p, (bh, q, k), &self.device)?,
            p2c: Tensor::from_vec(p2c, (bh, q, k), &self.device)?,
        };
        let mut cache = self.rel_idx_cache.lock().unwrap();
        if cache.len() >= 8 {
            cache.clear();
        }
    ...
    /// The (layer-normed) relative position embeddings: weights only, so computed once (gd-eok7).
    fn get_rel_embedding(&self) -> Result<Option<Tensor>> {
        if !self.relative_attention {
            return Ok(None);
        }
        if let Some(t) = self.rel_embeddings_cache.lock().unwrap().as_ref() {
        ...
        let rel_embeddings = self
            ...
            .embeddings()
            .clone();
        let out = if self.norm_rel_ebd.contains("layer_norm") {
            self.layer_norm
                .as_ref()
        ...
        *self.rel_embeddings_cache.lock().unwrap() = Some(out.clone());
        Ok(Some(out))
    }
}
...
pub(crate) fn make_log_bucket_position(
    ...
    let condition = lt_mid
        ...
        .mul(&gt_neg_mid.to_dtype(candle_core::DType::F32)?)?
        .to_dtype(DType::U8)?;
